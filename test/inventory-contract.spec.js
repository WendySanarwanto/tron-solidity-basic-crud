...
const mainNetConfig = require('../tronbox').networks.mainnet;
const config = require('../tronbox-config');

const { fullHost, privateKey } = mainNetConfig;
...
xdescribe('Inventory smartcontract', () => {      
    ...
    it(`should be exist in 'mainnet'`, async () => {
    ...
    it(`should allow admins to get Product in last row`, async () => {
    ...
    it(`should allow admins to update an existing product`, async() => {
    ...
    it(`should allow admins to delete an existing product by id`, async() => {
        assert.ok(createdProductId);
        const getProductInLastRowResponse = await inventoryContract.getProductInLastRow().call();
        assert.ok(Array.isArray(getProductInLastRowResponse) && (getProductInLastRowResponse.length === 5));
